...
{
    public class Mon2Connector : BaseConnector
    {
        ...
        public Mon2Connector(string monHost, int monPort, MessageReceived messageReceived, BaseConnector.MessageSended messageSended,
        ...
        public bool createConnection(out SocketAsyncEventArgs receiveSaea, out SocketAsyncEventArgs sendSaea)
        {
            ...
            {
                log.Error("Cannot establish connection to monitoring2.", e);
            }
            if (!socket.Connected)
            {
                return false;
