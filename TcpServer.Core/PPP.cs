...
namespace TcpServer.Core
{
    public class PPP
    {
        ...
        public static byte[] UnpackData(byte[] pByte)
        {
            var sDest = new List<byte>();
            var iLen = pByte.Length;
            var wPos = 0;
            while (wPos < iLen)
            {
                if (pByte[wPos] == 0x7D)
                {
                    sDest.Add((byte)(pByte[wPos + 1] ^ 0x20));
                    wPos += 2;
                }
                else
                    sDest.Add(pByte[wPos++]);
            }

            return sDest.ToArray();
        }

        public string PackData(string sSrc)
        {
            var sDest = "7E";
            var pByte = GetBytesFromByteString(sSrc);
            foreach (var b in pByte)
            {
                //if (b < 0x20)
                //{
                //    sDest += "7D";
                //    var bR = (byte)(b ^ 0x20);
                //    sDest += bR.ToString("X2");
                //}
                //else
                    if (b == 0x7E)
                {
        ...
        public string Make_PPP_Packet(string sSrc)
        {
            var sDest = "FF03"; // Признак PPP
            sDest += "7701"; // Протокол 0x0177
            sDest += sSrc;

            int wFSC = CalculateFCS(GetBytesFromByteString(sDest), PPPINITFCS);

            //sDest += wFSC.ToString("X4");

            var bByte = (byte)(wFSC & 0x00FF);
            sDest += bByte.ToString("X2");

            bByte = (byte)((wFSC >> 8) & 0x00FF);
            sDest += bByte.ToString("X2");

            //var crc = new Crc16Ibm();
            //var wfsc = (ushort)~crc.ComputeChecksumASCII(sDest);
            //sDest += wfsc.ToString("X4");
